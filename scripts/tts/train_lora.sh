#!/usr/bin/env bash
# LoRA fine-tuning launcher (single node).
#
# Usage:
#   scripts/tts/train_lora.sh [config.yaml]         # default: configs/tts/train/lora.yaml
#   NUM_GPUS=4 scripts/tts/train_lora.sh configs/tts/train/lora.yaml
#   ACCELERATE_CONFIG=... MASTER_PORT=29501 scripts/tts/train_lora.sh ...
#
...
# last checkpoint automatically (get_last_checkpoint in train_lora.py).

set -euo pipefail
cd "$(dirname "$0")/../.."

CONFIG="${1:-configs/tts/train/lora.yaml}"
...
# Inductor/dynamo tuning (compile is force-disabled for LoRA, but harmless)
...
exec accelerate launch \
    ...
    -m bodhan_genai.tts.training.train_lora "$CONFIG"
